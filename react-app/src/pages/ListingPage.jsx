        ...
        <div className="min-h-screen bg-[#f5f1ed] pb-32">
            ...
            <section className="pt-2 pb-1 bg-[#f5f1ed] relative overflow-hidden text-left border-b border-stone-200/40">
                <div className="container mx-auto px-6 relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-2">
                    <div>
                        <div className="inline-flex items-center gap-2 px-2 py-0.5 bg-stone-900/5 rounded-full text-[8px] font-black tracking-widest text-stone-500 uppercase mb-1">
                            LİSTELEME
                        </div>
                    ...
                    <p className="text-[10px] text-stone-500 font-serif italic max-w-md md:mb-1 opacity-80">
                ...
                <div className="container mx-auto px-6 flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        <button
                            ...
                            className="flex items-center gap-3 px-6 py-2.5 rounded-xl font-black text-[10px] tracking-widest uppercase transition-all bg-white border border-stone-100 text-stone-500 hover:border-stone-900 hover:text-stone-900"
                        >
                            <SlidersHorizontal className="w-4 h-4" />
                            FİLTRELE
                        </button>
                        <div className="hidden md:flex items-center gap-2 text-stone-500 font-black text-[9px] tracking-widest uppercase">
                            <span className="text-stone-900">{pagination.total}</span> İlan Bulundu
                        </div>
                    </div>

                    <div className="flex items-center gap-2 bg-stone-50 p-1 rounded-xl border border-stone-100">
                        <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500 hover:text-stone-600'}`}>
                            <LayoutGrid className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500 hover:text-stone-600'}`}>
                ...
                <div className={`absolute top-0 left-0 right-0 bg-white shadow-2xl rounded-b-[3rem] transition-transform duration-700 ease-out border-b border-stone-100 ${isFilterOpen ? 'translate-y-0' : '-translate-y-full'}`}>
                    <div className="container mx-auto max-w-7xl pt-10 pb-8 px-12">
                        ...
                        <div className="flex items-center justify-between mb-8 pb-6 border-b border-stone-50">
                            ...
                            <button onClick={() => setIsFilterOpen(false)} className="w-10 h-10 bg-stone-50 text-stone-500 rounded-xl flex items-center justify-center hover:bg-stone-900 hover:text-white transition-all active:scale-90">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        {group.options.map(opt => (
                                            <button
                                                ...
                                                className={`text-left px-4 py-2.5 rounded-xl font-serif italic text-sm transition-all border-2 ${filters[group.key] === opt ? 'bg-stone-900 border-stone-900 text-amber-400 font-bold shadow-md translate-x-1' : 'bg-stone-50 border-transparent text-stone-600 hover:bg-stone-100'}`}
                                            >
                                                {opt}
                                            </button>
                                        ))}
                        ...
                        <div className="mt-10 pt-6 border-t border-stone-50 flex items-center justify-between">
                            <div className="hidden md:flex items-center gap-2 text-[8px] font-black text-stone-500 uppercase tracking-[0.1em]">
            ...
            <div className="container mx-auto px-6 mt-6">
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-24 gap-4">
                        ...
                        <p className="text-stone-600 font-serif italic uppercase tracking-widest text-xs">İlanlar Getiriliyor...</p>
                    </div>
                ) : products.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-24 gap-4">
                        <Package className="w-12 h-12 text-stone-300" />
                        <p className="text-stone-600 font-serif italic text-lg">Bu grupta henüz ilan bulunmuyor.</p>
                    </div>
                ) : (
                    <div className={`grid gap-6 md:gap-8 ${viewMode === 'grid' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'grid-cols-1'}`}>
                        {products.map((ad, idx) => {
                            const fullImg = getFullImageUrl(ad.images?.[0]?.imageUrl);
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex-1 px-2 space-y-4">
                                        <div>
                                            <span className="text-[9px] font-black tracking-widest text-stone-500 uppercase mb-1 block">{categoryName}</span>
                    ...
                    <div className="mt-20 flex justify-center items-center gap-2">
                        {Array.from({ length: pagination.totalPages }, (_, i) => i + 1).map(p => (
                            <button
                                ...
                                className={`w-12 h-12 rounded-2xl font-black text-xs transition-all ${p === pagination.page ? 'bg-stone-900 text-amber-500 shadow-xl' : 'bg-white text-stone-400 hover:bg-stone-50'}`}
                            >
                                {p}
                            </button>
                        ))}
