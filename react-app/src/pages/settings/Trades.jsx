                                ...
                                <div key={t.id} className="bg-white rounded-2xl border border-stone-100 p-5 md:p-6 shadow-sm space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                        <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4">
                                            <div className="text-[9px] font-black text-amber-700 uppercase tracking-widest mb-3">
                                                {isReceiver ? 'SİZE TEKLİF EDİLEN' : 'SENİN VERDİKLERİN'}
                                            </div>
                                            <div className="space-y-3">
                                                {offer.map((it) => {
                                                    if (!it) return null;
                                                    const img = it.images?.[0]?.imageUrl ? getFullImageUrl(it.images[0].imageUrl) : null;
                                                    return (
                                                        <Link key={it.id} to={`/ilan/${it.id}`} className="flex items-center gap-3 group">
                                                            <div className="w-10 h-10 rounded-lg bg-white overflow-hidden shrink-0 border border-amber-100">
                                                                {img ? <img src={img} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-stone-50 flex items-center justify-center text-stone-300">?</div>}
                                                            </div>
                                                            <span className="font-bold text-stone-800 group-hover:text-amber-700 transition-colors leading-tight">{it.title}</span>
                                                        </Link>
                                                    );
                                                })}
                                            </div>
                                        ...
                                        <div className="bg-stone-50 border border-stone-100 rounded-2xl p-4">
                                            <div className="text-[9px] font-black text-stone-500 uppercase tracking-widest mb-3">
                                                {isReceiver ? 'KARŞILIĞINDA İSTENEN' : 'İSTEDİĞİN ÜRÜN'}
                                            </div>
                                            <div className="space-y-3">
                                                {request.map((it) => {
                                                    if (!it) return null;
                                                    ...
                                                    return (
                                                        <Link key={it.id} to={`/ilan/${it.id}`} className="flex items-center gap-3 group">
                                                            <div className="w-10 h-10 rounded-lg bg-white overflow-hidden shrink-0 border border-stone-200">
                                                                {img ? <img src={img} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-stone-50 flex items-center justify-center text-stone-300">?</div>}
                                                            </div>
                                                            <span className="font-bold text-stone-800 group-hover:text-amber-700 transition-colors leading-tight">{it.title}</span>
                                                        </Link>
