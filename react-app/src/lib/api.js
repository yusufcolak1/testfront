...
const API_URL = import.meta.env.VITE_API_URL?.startsWith('/') 
...
class ApiClient {
  ...
  async socialLogin(data) {
    const r = await this.post('/auth/social-login', data);
    if (r.data?.tokens?.accessToken) this.setToken(r.data.tokens.accessToken);
    return r;
  }
  ...
  // Soru-Cevap
  getItemQuestions(itemId) { return this.get(`/items/${itemId}/questions`); }
  askQuestion(itemId, question) { return this.post('/items/questions/ask', { itemId, question }); }
  answerQuestion(questionId, answer) { return this.patch(`/items/questions/${questionId}/answer`, { answer }); }
  ...
  // Algoritma — Eşleşme önerileri
  getTradeMatches(itemId) { return this.get(`/trades/matches/${itemId}`); }
  getTradeCycles(itemId) { return this.get(`/trades/cycles/${itemId}`); }
  ...
  // ---- Social/Comments ----
  getItemComments(itemId) { return this.get(`/items/${itemId}/comments`); }
  addComment(itemId, content) { return this.post(`/items/${itemId}/comments`, { content }); }

  // ---- Support ----
  sendSupportRequest(data) { return this.post('/support/request', data); }
