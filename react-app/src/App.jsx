...
function App() {
  ...
  const handleProtectedClick = (path, e) => {
    if (!isAuthenticated && path === '/ilan-ver') {
      e.preventDefault();
      openLoginModal();
    }
  };
      ...
      <div id="ui-container" className={showIntro ? "fixed top-[15vh] left-0 w-full pointer-events-none z-10 overflow-hidden opacity-0" : "relative z-10 w-full"}>
            ...
            <nav className="hidden lg:flex flex-1 justify-center items-center gap-2 lg:gap-6 transition-all duration-500 z-20 min-w-0 mx-4">
              {[
                ...
                <Link
                  key={item.path}
                  to={item.path}
                  onClick={(e) => handleProtectedClick(item.path, e)}
                  className={`flex items-center justify-center transition-all duration-500 relative group h-12 text-base font-serif italic font-medium tracking-wide shrink min-w-0
                    ${location.pathname === item.path ? 'text-stone-900' : 'text-stone-600 hover:text-stone-900'}
        ...
        <main className="pt-16 lg:pt-24 min-h-screen pb-16 lg:pb-0">
          ...
          <Routes>
            ...
            <Route path="/profil/premium-detay" element={settingsLoading ? null : (isPremiumEnabled ? <PremiumDetails /> : <Navigate to="/profil" replace />)} />
            ...
            <Route path="/admin" element={isAdmin ? <AdminLayout /> : <Navigate to="/" replace />}>
              ...
              <Route path="destek-talepleri" element={<AdminSupportRequests />} />
