...
const socialLogin = async ({ email, firstName, lastName, provider, providerId }) => {
  if (!email) throw new AppError('E-posta adresi gerekli.', 400);
  ...
  if (!user) {
    // Yeni kullanıcı oluştur (Sosyal girişle)
    user = await prisma.user.create({
      data: {
        email,
        passwordHash: 'SOCIAL_AUTH', // Şifreyle girişi engellemek için placeholder
        role: 'USER',
        status: 'ACTIVE',
        profile: {
...
module.exports = { register, login, refreshTokens, getMe, socialLogin };
